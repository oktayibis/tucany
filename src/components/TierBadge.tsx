...
export function StopTierBadge({ tier }: { readonly tier: StopTier }) {
  return (
    <span className="inline-block rounded-full border border-border px-1.5 py-0.5 font-display text-xs font-medium uppercase tracking-wide text-text-muted">
      {STOP_LABEL[tier]}
    </span>
  );
}
...
export function FoodTierBadge({ tier }: { readonly tier: FoodTier }) {
  return (
    <span
      className={`inline-block rounded-full px-1.5 py-0.5 font-display text-xs font-medium uppercase tracking-wide ${FOOD_TIER_STYLE[tier]}`}
    >
      {FOOD_LABEL[tier]}
    </span>
  );
